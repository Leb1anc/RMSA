...
RegionTy *MemRegionManager::getRegion(const A1 a1) {
  ...
  void *InsertPos;
  RegionTy *R =
      cast_or_null<RegionTy>(Regions.FindNodeOrInsertPos(ID, InsertPos));
...
TypedValueRegion::getExtent(SValBuilder &svalBuilder) const {
  ASTContext &Ctx = svalBuilder.getContext();
  QualType T = getDesugaredValueType(Ctx);

  if (isa<VariableArrayType>(T))
    return nonloc::SymbolVal(
        svalBuilder.getSymbolManager().getExtentSymbol(this));
...
void MemRegion::dump() const { dumpToStream(llvm::errs()); }
...
void VarRegion::dumpToStream(raw_ostream &os) const { os << *cast<VarDecl>(D); }

void RegionRawOffset::dump() const { dumpToStream(llvm::errs()); }
...
bool VarRegion::canPrintPrettyAsExpr() const { return true; }
    ...
    if (!STC)
      ...
      if (D->hasLocalStorage()) {
        ...
        if (isa<FunctionDecl>(STCD) || isa<ObjCMethodDecl>(STCD))
          ...
          const BlockTextRegion *BTR = getBlockTextRegion(
              BD, C.getCanonicalType(T), STC->getAnalysisDeclContext());
...
const BlockDataRegion *MemRegionManager::getBlockDataRegion(
    const BlockTextRegion *BC, const LocationContext *LC, unsigned blockCount) {
  const MemRegion *sReg = nullptr;
...
const CXXBaseObjectRegion *MemRegionManager::getCXXBaseObjectRegion(
    const CXXRecordDecl *RD, const MemRegion *Super, bool IsVirtual) {
...
bool MemRegion::isSubRegionOf(const MemRegion *R) const { return false; }
...
// void PtrRegion::setPtrState(Expr *initExpr, CheckerContext &C) {
//   ProgramStateRef state = C.getState();
//   SVal sval = state->getSVal(initExpr, C.getLocationContext());
//   if (!sval.getAs<Loc>()) {
//     fprintf(stderr, "非位置值无法赋值给指针");
//     return;
//   }
//   sval = sval.castAs<Loc>();
//   const MemRegion *region = sval.getAsRegion();
//   const MemSpaceRegion* spaceRegion = region->getMemorySpace();
//   if (spaceRegion->getAs<StackSpaceRegion>())
//     this->state = PtrState::toStack;
//   else if (spaceRegion->getAs<GlobalsSpaceRegion>())
//     this->state = PtrState::toGlobal;
//   else if (spaceRegion->getAs<HeapSpaceRegion>())
//     this->state = PtrState::toHeap;
//   else if (spaceRegion->getAs<UnknownSpaceRegion>())
//     this->state = PtrState::toUnknown;
//     else fprintf(stderr, "无法判断指针指向什么区域");
// }
