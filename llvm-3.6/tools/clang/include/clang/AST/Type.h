...
template <> class PointerLikeTypeTraits<::clang::Type *> {
public:
...
template <> class PointerLikeTypeTraits<::clang::ExtQuals *> {
public:
...
template <> struct isPodLike<clang::QualType> {
...
public:
  ...
  void setConst(bool flag) { Mask = (Mask & ~Const) | (flag ? Const : 0); }
...
public:
  ...
  std::string getAsString() const { return getAsString(split()); }
...
template <> class PointerLikeTypeTraits<clang::QualType> {
public:
...
protected:
  ...
  ArrayType(TypeClass tc, QualType et, QualType can, ArraySizeModifier sm,
            unsigned tq, bool ContainsUnexpandedParameterPack)
...
public:
  ...
  void Profile(llvm::FoldingSetNodeID &ID) {
    Profile(ID, getElementType(), getSize(), getSizeModifier(),
...
protected:
  FunctionType(TypeClass tc, QualType res, QualType Canonical, bool Dependent,
               bool InstantiationDependent, bool VariablyModified,
               bool ContainsUnexpandedParameterPack, ExtInfo Info)
...
public:
  struct ExceptionSpecInfo {
    ExceptionSpecInfo()
        : Type(EST_None), NoexceptExpr(nullptr), SourceDecl(nullptr),
...
protected:
  TypedefType(TypeClass tc, const TypedefNameDecl *D, QualType can)
      : Type(tc, can, can->isDependentType(),
             can->isInstantiationDependentType(), can->isVariablyModifiedType(),
             /*ContainsUnexpandedParameterPack=*/false),
...
public:
  ...
  static void Profile(llvm::FoldingSetNodeID &ID, const ASTContext &Context,
                      ...
                      const IdentifierInfo *Name, unsigned NumArgs,
                      const TemplateArgument *Args);
...
inline bool Type::isImageType() const {
  return isImage3dT() || isImage2dT() || isImage2dArrayT() || isImage1dT() ||
         isImage1dArrayT() || isImage1dBufferT();
}
...
template <typename T> struct ArrayType_cannot_be_used_with_getAs<T, true>;
